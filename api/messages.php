...
// ─── GET: Fetch messages for a post or welfare case ────────
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    $postId = (int)($_GET['post_id'] ?? 0);
    $caseId = (int)($_GET['case_id'] ?? 0);
    
    if (!$postId && !$caseId) {
        echo json_encode(['ok' => false, 'msg' => 'post_id or case_id required.']);
        exit;
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$receiverId || (!$postId && !$caseId) || !$message) {
        echo json_encode(['ok' => false, 'msg' => 'receiverId, message, and either postId or caseId are required.']);
        exit;
    }

    if ($caseId) {
        $stmt = $conn->prepare(
            "INSERT INTO messages (sender_id, receiver_id, welfare_case_id, message) VALUES (?, ?, ?, ?)"
        );
        $stmt->bind_param('iiis', $currentUserId, $receiverId, $caseId, $message);
    } else {
        $stmt = $conn->prepare(
